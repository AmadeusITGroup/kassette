  ...
  describe('logProperty', () => {
    it('should log string or scalar property', () => {
      ...
      expect(output).toEqual([{
        type: 'log',
        message: `- (cli | file | api | ${highlighted('default')}) test: ${highlighted('"text"')}`,
      }]);
    });
    ...
    it('should log provided properties', () => {
      const dummy = {};
      const {output} = createLogger();
      logProperty('provided', {
        value: dummy,
        origin: 'cli', // anything not default
      }, true);
      logProperty('not provided', {
        value: dummy,
        origin: 'default',
      }, true);
      expect(output).toEqual([
        ...
        {
          type: 'log',
          message: `- (cli | file | api | ${highlighted('default')}) not provided: ${highlighted('✗', 'red')}`,
        },
      ]);
    });
  });
